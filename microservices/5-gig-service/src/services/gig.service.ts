...
import { IRatingTypes, IReviewMessageDetails, ISellerDocument, ISellerGig } from "@salman-eng1/marketplace-shared";
...
const createGig = async (gig: ISellerGig): Promise<ISellerGig> => {
    ...
    if (createdGig) {
        ...
        await publishDirectMessage(
            ...
            'user-seller',
            JSON.stringify({ type: 'update-gig-count', gigSellerId: `${data.sellerId}` }),
...
const updateActiveGigProp = async (gigId: string, gigActive: boolean): Promise<ISellerGig> => {
    ...
    if (document) {
        const data: ISellerGig = document.toJSON?.() as ISellerGig
        await updateIndexedData('gigs', `${document.active}`, data)
...
export { getGigById, getSellerGigs, getSellerPausedGigs, createGig, updateGig, deleteGig, updateActiveGigProp, updateGigReview,seedData }