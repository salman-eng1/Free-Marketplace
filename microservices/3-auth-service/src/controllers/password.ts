...
import { BadRequestError, IAuthDocument, IEmailMessageDetails } from "@salman-eng1/marketplace-shared";
...
throw new BadRequestError('Invalid credentials', 'password create() method error')
    ...
    const randomBytes: Buffer= await Promise.resolve(crypto.randomBytes(req.body))
 ...
 res.status(StatusCodes.OK).json({message: 'password reset email has been sent'})
}
