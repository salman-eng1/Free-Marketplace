...
import { IMessageDocument, IOrderDocument, IOrderNotifcation, winstonLogger } from '@salman-eng1/marketplace-shared';
...
let chatSocketClient: SocketClient;  // we don't want it to be initiated inside the constructor
export class SocketIOAppHandler {
  ...
  private chatSocketServiceIOConnections(): void {
    chatSocketClient = io(`${config.MESSAGE_BASE_URL}`, {
      transports: ['websocket', 'polling'],  // if websocket is not available, it will use polling
      secure: true
    });
