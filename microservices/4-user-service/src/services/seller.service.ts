import { IOrderMessage, IRatingTypes, IReviewMessageDetails, ISellerDocument } from "@salman-eng1/marketplace-shared";
...
const getRandomSellers= async (count: number): Promise<ISellerDocument[] | null> =>{
    const sellers: ISellerDocument[] | null = await SellerModel.aggregate([{
...
const updateSellerOngoingJobsProp=async (sellerId: string, ongoingJobs: string): Promise<void> => {
    await SellerModel.updateOne(
        {_id: sellerId,},
        {$inc: {ongoingJobs}}
    ).exec()
