...
import { IBuyerDocument, ISellerDocument, winstonLogger } from '@salman-eng1/marketplace-shared';
...
const consumeBuyerDirectMessage = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'marketplace-buyer-update';
...
const consumeSellerDirectMessage = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'marketplace-seller-update';
...
const consumeReviewFanoutMessages = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'marketplace-review';
...
const consumeSeedGigDirectMessages = async (channel: Channel): Promise<void> => {
  try {
    ...
    const exchangeName = 'marketplace-gig';
    ...
    channel.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
      ...
      if (type === 'getSellers') {
        ...
        const sellers: ISellerDocument[] = await getRandomSellers(parseInt(count, 10)) as ISellerDocument[]
        await publishDirectMessage(
          channel,
          'marketplace-seed-gig',
          'receive-sellers',
